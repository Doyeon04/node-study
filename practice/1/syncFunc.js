...
try {
  ...
  const fileId = fs.openSync(
    `.${path.sep + folderName + path.sep + filename1}`,
    "w"
  );
  console.log(
    `${folderName + path.sep + filename1} file (${fileId}) is available.\n`
  );

  // 3. 생성한 파일명 work1.txt을 homework1.txt로 변경하고, 결과를 출력합니다.
  fs.renameSync(
    `.${path.sep + folderName + path.sep + filename1}`,
    `.${path.sep + folderName + path.sep + filename2}`
  );
  console.log(
  ...
  fs.writeFileSync(`.${path.sep + folderName + path.sep + filename2}`, buffer);
